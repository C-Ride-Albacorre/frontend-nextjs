  ...
  return (
    <>
      ...
      <div className="mt-8 flex justify-center">
        <div className="flex rounded-2xl border border-border px-1  py-2 md:px-2  gap-2 bg-foreground-200 text-xs md:text-sm text-nowrap ">
          {(Object.keys(TABS_CONTENT) as TabKey[]).map((key) => (
            <button
              key={key}
              onClick={() => setActiveTab(key)}
              className={` px-2 md:px-4 py-3 rounded-xl transition-all cursor-pointer ${
                activeTab === key
